...
static bool select_col(uint8_t col) {
    ...
    if (col < DIRECT_COL_NUM) {
        ...
        // if (col == DIRECT_COL_NUM) HC595_output_bit(0x0);
        return true;
    }
    return false;
}
...
static void unselect_col(uint8_t col) {
    ...
    if (col < DIRECT_COL_NUM) {
        ...
        if (col >= (MATRIX_COLS - DIRECT_COL_NUM - 1)) HC595_output(0xFFFF);
        // HC595_output_bit(0x1);
    }
}

static void unselect_cols(void) {
    for (uint8_t x = 0; x < DIRECT_COL_NUM; x++) {
        if (col_pins[x] != NO_PIN) {
            setPinInputHigh_atomic(col_pins[0]);
        }
    }
    HC595_output(0xFFFF);
}
...
void select_all_cols(void) {
    for (uint8_t x = 0; x < DIRECT_COL_NUM; x++) {
        if (col_pins[0] != NO_PIN) {
            setPinOutput_writeLow(col_pins[0]);
        }
    }
    HC595_output(0x0000);
}
